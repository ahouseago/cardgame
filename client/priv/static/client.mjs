...
// build/dev/javascript/shared/card.mjs
var Attack = class extends CustomType {
};
var Counter = class extends CustomType {
};
function to_string7(card) {
  if (card instanceof Attack) {
    return "attack";
  } else if (card instanceof Counter) {
    return "counter";
  } else {
    return "rest";
  }
}
function to_json(card) {
  return string2(to_string7(card));
}

// build/dev/javascript/shared/messaging.mjs
...
var Challenging = class extends CustomType {
  constructor(opponent_id) {
    super();
    this.opponent_id = opponent_id;
  }
};
var InMatch = class extends CustomType {
  constructor(match_id) {
    super();
    this.match_id = match_id;
  }
};
var MatchEnded = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var Chat = class extends CustomType {
  constructor(to, x1) {
    super();
    this.to = to;
    this[1] = x1;
...
var ChallengeRequest = class extends CustomType {
  constructor(target) {
    super();
    this.target = target;
  }
};
var ChallengeResponse = class extends CustomType {
  constructor(challenger, accepted) {
    super();
    this.challenger = challenger;
    this.accepted = accepted;
  }
};
var PlayCard = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var PhaseUpdate = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var Direct = class extends CustomType {
  constructor(from3, x1) {
    super();
    this.from = from3;
    this[1] = x1;
  }
};
var Broadcast = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var Challenge = class extends CustomType {
  constructor(from3) {
    super();
    this.from = from3;
  }
};
var ChallengeAccepted = class extends CustomType {
};
...
function incoming_to_json(msg) {
    ...
    let _pipe = (() => {
      if (msg instanceof Chat) {
        ...
        return [
          "chat",
          object2(
            toList([["to", int2(from3)], ["text", string2(text2)]])
          )
        ];
      } else if (msg instanceof ChallengeRequest) {
        let target = msg.target;
        return [
          "challenge",
          object2(toList([["target", int2(target)]]))
        ];
      } else if (msg instanceof ChallengeResponse) {
        let challenger = msg.challenger;
        let accepted = msg.accepted;
        return [
          "challengeResponse",
          object2(
            toList([
              ["challenger", int2(challenger)],
        ...
        return [
          "playCard",
          object2(toList([["card", to_json(card)]]))
        ];
      } else {
        let card = msg[0];
        return ["card", to_json(card)];
      }
    })();
  ...
  let message = $[1];
  return to_string5(
    object2(toList([["type", t], ["message", message]]))
  );
}
function phase_from_json(phase) {
  let phase_type = (() => {
    let _pipe = phase;
    return field("phaseType", string)(_pipe);
  })();
  if (phase_type.isOk() && phase_type[0] === "idle") {
    return new Ok(new Idle());
  } else if (phase_type.isOk() && phase_type[0] === "challenging") {
    let $ = field("phaseDetail", int)(phase);
    if ($.isOk()) {
      let id = $[0];
      return new Ok(new Challenging(id));
    } else {
      let err = $[0];
      return new Error(err);
    }
  } else if (phase_type.isOk() && phase_type[0] === "inMatch") {
    let $ = field("phaseDetail", int)(phase);
    if ($.isOk()) {
      let id = $[0];
      return new Ok(new InMatch(id));
    } else {
      let err = $[0];
      return new Error(err);
    }
  } else if (phase_type.isOk()) {
    let other = phase_type[0];
    return new Error(
      toList([
        new DecodeError(
          "unknown phase type in status update",
          other,
          toList(["message", "status", "phase"])
        )
      ])
    );
  } else {
    let any2 = phase_type[0];
    return new Error(any2);
  }
}
function result_from_json(result) {
  let result_type = (() => {
    let _pipe = result;
    return field("resultType", string)(_pipe);
  })();
  if (result_type.isOk() && result_type[0] === "draw") {
    return new Ok(new RoundResult(new MatchEnded(new Draw())));
  } else if (result_type.isOk() && result_type[0] === "victory") {
    let winner = (() => {
      let _pipe = result;
      return field("winner", int)(_pipe);
    })();
    if (winner.isOk()) {
      let winner_id = winner[0];
      return new Ok(new RoundResult(new MatchEnded(new Victory(winner_id))));
    } else {
      let err = winner[0];
      return new Error(err);
    }
    ...
    throw makeError(
      ...
      217,
      "result_from_json",
      "this is probably next round, should I check for that explicitely?",
      {}
    );
  }
}
function msg_from_json(text2) {
  let msg_with_type = (() => {
    let _pipe = text2;
    return decode3(
      _pipe,
  ...
  if (msg_with_type.isOk() && msg_with_type[0][0] === "error") {
    let msg = msg_with_type[0][1];
    return decode1(
      (var0) => {
        return new Err(var0);
      },
  ...
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "chat") {
    let msg = msg_with_type[0][1];
    let _pipe = msg;
    return decode2(
      (var0, var1) => {
  ...
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "broadcast") {
    let msg = msg_with_type[0][1];
    return decode1(
      (var0) => {
        return new Broadcast(var0);
      },
      string
    )(msg);
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "challenge") {
    let msg = msg_with_type[0][1];
    let _pipe = msg;
    return decode1(
      (var0) => {
        return new Challenge(var0);
      },
      field("from", int)
    )(_pipe);
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "challengeAccepted") {
    return new Ok(new ChallengeAccepted());
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "status") {
    let msg = msg_with_type[0][1];
    return decode1(
      (var0) => {
        return new PhaseUpdate(var0);
      },
      field("phase", phase_from_json)
    )(msg);
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "result") {
    let msg = msg_with_type[0][1];
    return result_from_json(msg);
  } else if (msg_with_type.isOk() && msg_with_type[0][0] === "connected") {
    let msg = msg_with_type[0][1];
    return decode1(
      (var0) => {
        return new Connected(var0);
      },
      int
    )(msg);
  } else if (msg_with_type.isOk()) {
    let found = msg_with_type[0][0];
    return new Error(
      toList([
        new DecodeError(
  ...
  } else if (!msg_with_type.isOk() && msg_with_type[0] instanceof UnexpectedFormat) {
    let e = msg_with_type[0][0];
    return new Error(e);
  } else if (!msg_with_type.isOk() && msg_with_type[0] instanceof UnexpectedByte) {
    let byte = msg_with_type[0].byte;
    return new Error(toList([new DecodeError(byte, "", toList([]))]));
  } else if (!msg_with_type.isOk() && msg_with_type[0] instanceof UnexpectedSequence) {
    let byte = msg_with_type[0].byte;
    return new Error(toList([new DecodeError(byte, "", toList([]))]));
  } else {
    return new Error(
      toList([new DecodeError("chat or broadcast", "", toList([]))])
    );
  }
}
function dynamic_err_to_string(errs) {
  return fold(
    errs,
    "",
    (err_string, err) => {
...
var Connected2 = class extends CustomType {
  constructor(socket, id, messages) {
    super();
    this.socket = socket;
    this.id = id;
...
var WsConnected = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var RecieveTextMessage = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var Disconnect = class extends CustomType {
};
var FailedToConnect = class extends CustomType {
};
var Err2 = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var SendMessage = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var SendChatMessage = class extends CustomType {
};
function init3(_) {
  return [new Disconnected(), none()];
}
...
function init_ws() {
  return init2(
    ...
    (event2) => {
      ...
      if (event2 instanceof InvalidUrl) {
        println("ws connect error: invalid url");
        return new FailedToConnect();
      } else if (event2 instanceof OnOpen) {
        let ws = event2[0];
        return new WsConnected(ws);
      } else if (event2 instanceof OnTextMessage) {
        let message = event2[0];
        let $ = msg_from_json(message);
        if ($.isOk()) {
          let msg = $[0];
          return new RecieveTextMessage(msg);
        } else {
          let any2 = $[0];
          println("failed to decode websocket message:");
          debug(any2);
          return new Err2(dynamic_err_to_string(any2));
        }
      } else if (event2 instanceof OnBinaryMessage) {
        let bit_array = event2[0];
        throw makeError(
          "todo",
          "client",
          61,
...
function chat_message(model) {
  throw makeError(
    "todo",
    "client",
    130,
    "chat_message",
    "get the text and recipient from the model (currently isn't added)",
    {}
  );
}
function update2(model, msg) {
  if (msg instanceof Connect) {
    return [model, init_ws()];
  } else if (msg instanceof WsConnected) {
    let socket = msg[0];
    return [new Connected2(socket, 0, toList([])), none()];
  } else if (model instanceof Connected2 && msg instanceof RecieveTextMessage) {
    let socket = model.socket;
    let id = model.id;
    let msgs = model.messages;
    let msg$1 = msg[0];
    if (msg$1 instanceof Err) {
      let err_string = msg$1[0];
      return [
        new Connected2(socket, id, prepend(err_string, msgs)),
        none()
      ];
    } else if (msg$1 instanceof PhaseUpdate) {
      let phase = msg$1[0];
      throw makeError(
        "todo",
        "client",
    ...
    } else if (msg$1 instanceof Direct) {
      let from3 = msg$1.from;
      let message = msg$1[1];
      return [
        new Connected2(
          socket,
          id,
          prepend("from " + to_string2(from3) + ": " + message, msgs)
        ),
        none()
      ];
    } else if (msg$1 instanceof Broadcast) {
      let message = msg$1[0];
      return [
        new Connected2(socket, id, prepend("broadcast: " + message, msgs)),
        none()
      ];
    } else if (msg$1 instanceof Challenge) {
      let from3 = msg$1.from;
      return [
        new Connected2(
          socket,
          id,
          prepend(to_string2(from3) + " has challenged you!", msgs)
        ),
        none()
      ];
    } else if (msg$1 instanceof ChallengeAccepted) {
      throw makeError(
        "todo",
        "client",
        104,
    ...
    } else if (msg$1 instanceof RoundResult) {
      let result = msg$1[0];
      throw makeError(
        "todo",
        "client",
      ...
      let id$1 = msg$1.id;
      return [new Connected2(socket, id$1, msgs), none()];
    }
  } else if (msg instanceof Err2) {
    let msg$1 = msg[0];
    println("got error message: " + msg$1);
    return [model, none()];
  } else if (msg instanceof RecieveTextMessage) {
    return [model, none()];
  } else if (msg instanceof Disconnect) {
    return [model, none()];
  } else if (msg instanceof FailedToConnect) {
    return [model, none()];
  } else if (model instanceof Connected2 && msg instanceof SendMessage) {
    let socket = model.socket;
    let msg$1 = msg[0];
    return [model, send(socket, incoming_to_json(msg$1))];
  } else if (msg instanceof SendMessage) {
    return [model, none()];
  } else if (model instanceof Connected2 && msg instanceof SendChatMessage) {
    let socket = model.socket;
    return [
      model,
      send(socket, incoming_to_json(chat_message(model)))
    ];
  } else {
    return [model, none()];
  }
...
function send_message(id, msg) {
  return new SendMessage(new Chat(id, msg));
}
function view(model) {
  if (model instanceof Disconnected) {
    return div(
      toList([]),
      toList([
        h1(toList([]), toList([text("Disconnected")])),
        button(
          toList([on_click(new Connect())]),
          toList([text("Connect to server")])
        )
      ])
    );
  } else {
    ...
    return body(
      ...
      toList([
        h1(
          toList([]),
          toList([text("Welcome player " + to_string2(id))])
        ),
        ul(
          toList([]),
          map2(
            msgs,
            (msg) => {
              return li(toList([]), toList([text(msg)]));
            }
          )
        ),
        form(
          toList([on_submit(new SendChatMessage())]),
          toList([
            input(
              toList([
                on_input(
                  (_capture) => {
                    return send_message(id, _capture);
                  }
                ),
